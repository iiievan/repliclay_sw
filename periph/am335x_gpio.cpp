...
void  am335x_gpio::dir_mode_set(uint32_t pinnum, REGS::GPIO::e_PINDIR pindir)
{
    if(pinnum > 31)
        return;
...
{    
    ...
    m_regs.DEBOUNCENABLE.b.DEBOUNCEENABLE &= ~(1 << pinnum);

    // Programming the DEBOUNCEENABLE[n] bit in GPIO_DEBOUNCEENABLE register.
    //HWREG(baseAdd + GPIO_DEBOUNCENABLE) |= (ctrl_flag  << pinnum);
    m_regs.DEBOUNCENABLE.b.DEBOUNCEENABLE |= (ctrl_flag << pinnum);
}

void  am335x_gpio::debounce_time_config(uint32_t debounce_time)
{
